                    ...
                    <div class="payment-prev-card d-flex flex-wrap align-items-center">
                        ...
                        <div class="content">
                            <ul class="payment-prev-card-list d-flex flex-wrap align-items-center">
                                ...
                                @if($promocode==1)
                                    <li>
                                        @lang('Promocode bonus')
                                        <b> {{ $ammount }}</b> {{$data->baseCurrency()}}
                                    </li>
                                @endif
                            </ul>
                            @if($data->method_code<1000)
                                <a href="{{route('user.deposit.confirm')}}" target="_blank" class="btn btn-block py-3 font-weight-bold mt-4 cmn-btn">@lang('Confirm')</a>
                            @else
                                <a href="{{route('user.deposit.manual.confirm')}}" target="_blank" class="btn btn-block py-3 font-weight-bold mt-4 cmn-btn">@lang('Confirm')</a>
                            @endif
                        </div>
                    </div>
